...
export default {
  ...
  mounted() {
    ...
    if (register) {
      ...
      this.loginForm.adminname = register.adminname;
  ...
  methods: {
    ...
    //登录成功获取登录信息
    getcodeuser(adminname) {
      this.$api.GetCodeAdmin({ adminname }).then((res) => {
        if (res.data.status == 200) {
          ...
          let obj = {
            ...
            token: data.id + this.uuid + data.adminname,
            avatar: data.avatar,
            email: data.email,
            id: data.id,
            type: data.type,
          ...
          if (res.data.status == 200) {
            ...
            if (time >= 300000) {
              ...
              // }
                ...
                console.log(res.data.data[0].loginname, "嘿嘿嘿");
                this.status = 2;
                this.getcodeuser(res.data.data[0].loginname);
                clearInterval(this.timer);

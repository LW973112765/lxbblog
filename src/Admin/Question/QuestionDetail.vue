<template>
    <transition appear name="animate__animated animate__bounce" enter-active-class="animate__backInUp"
      ...
      <div class="main">
        <div class="content">
          <header><h2>用户{{ username }}于{{sendtime}}提交</h2></header>
          <div class="qu-content">
            <section class="qu-item" v-for="(item, index) in questions" :key="index">
              <h3>{{ `Q${index + 1}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${item.topic}` }}
                <span v-if="item.isMandatory"> *</span>
              </h3>
              <textarea disabled rows="8" cols="80" v-if="item.type === 'textarea'" v-model="item.answer"
                required  >
                        </textarea>
              <ul v-else class="options-list">
                <li v-for="(option, optIndex) in item.options" :key="optIndex">
                  <label>
                    <input disabled class="checked"  
                    required :checked="item.answer.includes(optIndex)" v-if="item.type === 'radio'" :type="item.type" :name="index + 1"
                 >
                    <input disabled required :checked="item.answer.includes(optIndex)" v-else :type="item.type" :name="index + 1"
                 >
                    <span>{{ option }}</span>
                    <!-- :style="`color: ${
  ...
  export default {
    ...
    mounted() {
        this.quData = this.$route.query.quData
        this.questions = this.$route.query.questions
        this.username = this.$route.query.username
        this.sendtime = this.$route.query.sendtime
      console.log(this.quData,'qudata');
      // console.log(this.$route.query,'langwei');
  }
   ,
    methods: {
      getData() {
